...
PassOwnPtr<JSScheduledAction> JSScheduledAction::create(ExecState* exec, DOMWrapperWorld* isolatedWorld, ContentSecurityPolicy* policy)
{
    JSValue v = exec->argument(0);
    CallData callData;
    if (getCallData(v, callData) == CallTypeNone) {
        if (policy && !policy->allowEval(JSScriptState::forExecState(exec)))
            return nullptr;
        String string = v.toString(exec)->value(exec);
        if (exec->hadException())
            return nullptr;
        return adoptPtr(new JSScheduledAction(string, isolatedWorld));
    }

    return adoptPtr(new JSScheduledAction(exec, v, isolatedWorld));
}
    
JSScheduledAction::~JSScheduledAction()
{
}

JSScheduledAction::JSScheduledAction(ExecState* exec, JSValue function, DOMWrapperWorld* isolatedWorld)
...
void JSScheduledAction::executeFunctionInContext(JSGlobalObject* globalObject, JSValue thisValue, ScriptExecutionContext* context)
{
    ASSERT(m_function);
...
void JSScheduledAction::execute(Document* document)
{
    ...
    if (m_function)
        ...
        frame->script(JSScriptType)->executeScriptInWorld(m_isolatedWorld.get(), m_code);
}

#if ENABLE(WORKERS)
void JSScheduledAction::execute(WorkerContext* workerContext)
{
