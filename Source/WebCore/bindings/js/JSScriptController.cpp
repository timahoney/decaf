...
JSScriptController::JSScriptController(Frame* frame)
...
JSScriptController::~JSScriptController()
{
...
void JSScriptController::destroyWindowShell(DOMWrapperWorld* world)
{
...
JSDOMWindowShell* JSScriptController::createWindowShell(DOMWrapperWorld* world)
{
    ASSERT(!m_windowShells.contains(world));
...
ScriptValue JSScriptController::evaluateInWorld(const ScriptSourceCode& sourceCode, DOMWrapperWorld* world)
{
...
ScriptValue JSScriptController::evaluate(const ScriptSourceCode& sourceCode) 
{
    return evaluateInWorld(sourceCode, mainThreadNormalWorld());
}

PassRefPtr<DOMWrapperWorld> JSScriptController::createWorld()
{
    return DOMWrapperWorld::create(JSDOMWindow::commonJSGlobalData());
}

void JSScriptController::getAllWorlds(Vector<RefPtr<DOMWrapperWorld> >& worlds)
{
    static_cast<WebCoreJSClientData*>(JSDOMWindow::commonJSGlobalData()->clientData)->getAllWorlds(worlds);
}

void JSScriptController::clearWindowShell(DOMWindow* newDOMWindow, bool goingIntoPageCache)
{
    if (m_windowShells.isEmpty())
        return;
...
JSDOMWindowShell* JSScriptController::initScript(DOMWrapperWorld* world)
{
    ASSERT(!m_windowShells.contains(world));
...
TextPosition JSScriptController::eventHandlerPosition() const
{
...
void JSScriptController::enableEval()
{
...
void JSScriptController::disableEval(const String& errorMessage)
{
    JSDOMWindowShell* windowShell = existingWindowShell(mainThreadNormalWorld());
    if (!windowShell)
        return;
...
void JSScriptController::attachDebugger(ScriptDebugServer* debugger)
{
    for (ShellMap::iterator iter = m_windowShells.begin(); iter != m_windowShells.end(); ++iter)
        attachDebugger(iter->value.get(), debugger);
}

void JSScriptController::attachDebugger(JSDOMWindowShell* shell, ScriptDebugServer* debugger)
{
    if (!shell)
        return;
    
    ASSERT(!debugger || debugger->scriptType() == JSScriptType);
    
    // FIXME: The attach(JSGlobalObject*) function should not be a part of ScriptDebugServer.
    // Instead, it should just be a part of JSScriptDebugServer, and we should be able to cast
    // the ScriptDebugServer to a JSScriptDebugServer. However, when we reinterpret_cast
    // the ScriptDebugServer, some problems happen underneath, and we crash!
    // Fix this somehow.
...
void JSScriptController::updateDocument()
{
    for (ShellMap::iterator iter = m_windowShells.begin(); iter != m_windowShells.end(); ++iter) {
...
void JSScriptController::updateSecurityOrigin()
{
    // Our bindings do not do anything in this case.
}

Bindings::RootObject* JSScriptController::cacheableBindingRootObject()
{
...
Bindings::RootObject* JSScriptController::bindingRootObject()
{
    if (!canExecuteScripts(NotAboutToExecuteScript))
        return 0;
...
PassRefPtr<Bindings::RootObject> JSScriptController::createRootObject(void* nativeHandle)
{
...
#if ENABLE(INSPECTOR)
void JSScriptController::setCaptureCallStackForUncaughtExceptions(bool)
{
}

void JSScriptController::collectIsolatedContexts(Vector<std::pair<ScriptState*, SecurityOrigin*> >& result)
{
    for (ShellMap::iterator iter = m_windowShells.begin(); iter != m_windowShells.end(); ++iter) {
        JSC::ExecState* exec = iter->value->window()->globalExec();
        SecurityOrigin* origin = iter->value->window()->impl()->document()->securityOrigin();
        result.append(std::pair<ScriptState*, SecurityOrigin*>(JSScriptState::forExecState(exec), origin));
    }
}
...
NPObject* JSScriptController::windowScriptNPObject()
{
...
NPObject* JSScriptController::createScriptObjectForPluginElement(HTMLPlugInElement* plugin)
{
...
PassRefPtr<JSC::Bindings::Instance> JSScriptController::createScriptInstanceForWidget(Widget* widget)
{
...
JSObject* JSScriptController::jsObjectForPluginElement(HTMLPlugInElement* plugin)
{
...
void JSScriptController::cleanupScriptObjectsForPlugin(void* nativeHandle)
{
    RootObjectMap::iterator it = m_rootObjects.find(nativeHandle);
...
void JSScriptController::clearScriptObjects()
{
    JSLockHolder lock(JSDOMWindowBase::commonJSGlobalData());
...
bool JSScriptController::executeIfJavaScriptURL(const KURL& url, ShouldReplaceDocumentIfJavaScriptURL shouldReplaceDocumentIfJavaScriptURL)
{
    ...
    if (!result.getString(JSScriptState::forExecState(exec), scriptResult))
        return true;
#else
